...
public class GenericVIM extends Vim {
    ...
    public GenericVIM(String type, String username, String password, String brokerIp, String managementPort, ApplicationContext context) throws PluginException {
        super(type, username, password, brokerIp, managementPort, context);
    }

    public GenericVIM(String type, String name, int port, String managementPort, ApplicationContext context) throws PluginException {
        super(type, name, port, managementPort, context);
    }

    public GenericVIM() {
    }

    public GenericVIM(String type, String name, String managementPort, ApplicationContext context) throws PluginException {
    ...
    public List<DeploymentFlavour> queryDeploymentFlavors(VimInstance vimInstance) throws VimException {
        try {
            ...
            for (DeploymentFlavour flavour : flavors)
                log.debug("\t" + flavour.getFlavour_key());
            return flavors;
        } catch (Exception e) {
            if (log.isDebugEnabled()) {
    ...
    public Network update(VimInstance vimInstance, Network updatingNetwork) throws VimException {
        ...
        try {
            log.debug("Updating Network with name: " + updatingNetwork.getName() + " on VimInstance " + vimInstance.getName());
            updatedNetwork = client.updateNetwork(vimInstance, updatingNetwork);
            log.info("Updated Network with name: " + updatingNetwork.getName() + " on VimInstance " + vimInstance.getName());
        } catch (Exception e) {
            if (log.isDebugEnabled()) {
                log.error("Not updated Network with name: " + updatingNetwork.getName() + " successfully on VimInstance " + vimInstance.getName() + ". Caused by: " + e.getMessage(), e);
            } else {
                log.error("Not updated Network with name: " + updatingNetwork.getName() + " successfully on VimInstance " + vimInstance.getName() + ". Caused by: " + e.getMessage());
            }
            throw new VimException("Not updated Network with name: " + updatingNetwork.getName() + " successfully on VimInstance " + vimInstance.getName() + ". Caused by: " + e.getMessage(), e);
        }
        log.debug("Updating Subnets for Network with name: " + updatingNetwork.getName() + " on VimInstance " + vimInstance.getName() + " -> " + updatingNetwork.getSubnets());
        ...
        for (Subnet subnet : updatingNetwork.getSubnets()) {
            if (subnet.getExtId() != null) {
                try {
                    log.debug("Updating Subnet with name: " + subnet.getName() + " on Network with name: " + updatingNetwork.getName() + " on VimInstance " + vimInstance.getName());
                    Subnet updatedSubnet = client.updateSubnet(vimInstance, updatedNetwork, subnet);
                    log.info("Updated Subnet with name: " + subnet.getName() + " on Network with name: " + updatingNetwork.getName() + " on VimInstance " + vimInstance.getName());
                    ...
                    if (log.isDebugEnabled()) {
                        log.error("Not updated Subnet with name: " + subnet.getName() + " successfully on Network with name: " + updatingNetwork.getName() + " on VimInstance " + vimInstance.getName() + ". Caused by: " + e.getMessage(), e);
                    } else {
                        log.error("Not updated Subnet with name: " + subnet.getName() + " successfully on Network with name: " + updatingNetwork.getName() + " on VimInstance " + vimInstance.getName() + ". Caused by: " + e.getMessage());
                    }
                    throw new VimException("Not updated Subnet with name: " + subnet.getName() + " successfully on Network with name: " + updatingNetwork.getName() + " on VimInstance " + vimInstance.getName() + ". Caused by: " + e.getMessage(), e);
                }
            } else {
                try {
                    log.debug("Creating Subnet with name: " + subnet.getName() + " on Network with name: " + updatingNetwork.getName() + " on VimInstance " + vimInstance.getName());
                    Subnet createdSubnet = client.createSubnet(vimInstance, updatedNetwork, subnet);
                    log.info("Created Subnet with name: " + subnet.getName() + " on Network with name: " + updatingNetwork.getName() + " on VimInstance " + vimInstance.getName());
                    ...
                    if (log.isDebugEnabled()) {
                        log.error("Not created Subnet with name: " + subnet.getName() + " successfully on Network with name: " + updatingNetwork.getName() + " on VimInstance " + vimInstance.getName() + ". Caused by: " + e.getMessage(), e);
                    } else {
                        log.error("Not created Subnet with name: " + subnet.getName() + " successfully on Network with name: " + updatingNetwork.getName() + " on VimInstance " + vimInstance.getName() + ". Caused by: " + e.getMessage());
                    }
                    throw new VimException("Not created Subnet with name: " + subnet.getName() + " successfully on Network with name: " + updatingNetwork.getName() + " on VimInstance " + vimInstance.getName() + ". Caused by: " + e.getMessage(), e);
                }
            }
        }
        ...
        try {
            log.debug("Listing all Subnet IDs of Network with name: " + updatingNetwork.getName() + " on VimInstance " + vimInstance.getName());
            existingSubnetExtIds = client.getSubnetsExtIds(vimInstance, updatedNetwork.getExtId());
            log.info("Listed all Subnet IDs of Network with name: " + updatingNetwork.getName() + " on VimInstance " + vimInstance.getName() + " -> Subnet IDs: " + existingSubnetExtIds);
        } catch (Exception e) {
            if (log.isDebugEnabled()) {
                log.error("Not listed Subnets of Network with name: " + updatingNetwork.getName() + " successfully of VimInstance " + vimInstance.getName() + ". Caused by: " + e.getMessage(), e);
            } else {
                log.error("Not listed Subnets of Network with name: " + updatingNetwork.getName() + " successfully of VimInstance " + vimInstance.getName() + ". Caused by: " + e.getMessage());
            }
            throw new VimException("Not listed Subnets of Network with name: " + updatingNetwork.getName() + " successfully of VimInstance " + vimInstance.getName() + ". Caused by: " + e.getMessage(), e);
        }
        for (String existingSubnetExtId : existingSubnetExtIds) {
            if (!updatedSubnetExtIds.contains(existingSubnetExtId)) {
                try {
                    log.debug("Deleting Subnet with id: " + existingSubnetExtId + " on Network with name: " + updatingNetwork.getName() + " on VimInstance " + vimInstance.getName());
                    client.deleteSubnet(vimInstance, existingSubnetExtId);
                    log.info("Deleted Subnet with id: " + existingSubnetExtId + " on Network with name: " + updatingNetwork.getName() + " on VimInstance " + vimInstance.getName());
                } catch (Exception e) {
                    if (log.isDebugEnabled()) {
                        log.error("Not Deleted Subnet with id: " + existingSubnetExtId + " successfully on Network with name: " + updatingNetwork.getName() + " on VimInstance " + vimInstance.getName() + ". Caused by: " + e.getMessage(), e);
                    } else {
                        log.error("Not Deleted Subnet with id: " + existingSubnetExtId + " successfully on Network with name: " + updatingNetwork.getName() + " on VimInstance " + vimInstance.getName() + ". Caused by: " + e.getMessage());
                    }
                    throw new VimException("Not Deleted Subnet with id: " + existingSubnetExtId + " successfully on Network with name: " + updatingNetwork.getName() + " on VimInstance " + vimInstance.getName() + ". Caused by: " + e.getMessage(), e);
                }
            }
        }
        log.info("Subnets of Network with name: " + updatingNetwork.getName() + " updated successfully on VimInstance " + vimInstance.getName());
    ...
    public Network query(VimInstance vimInstance, String extId) throws VimException {
        try {
            log.debug("Finding Network with extId: " + extId + " on VimInstance " + vimInstance.getName());
            Network network = client.getNetworkById(vimInstance, extId);
            log.info("Found Network with extId: " + network.getId() + " on VimInstance " + vimInstance.getName() + " -> Network: " + network);
            return network;
        } catch (Exception e) {
            if (log.isDebugEnabled()) {
                log.error("Not found Network with extId: " + extId + " successfully on VimInstance " + vimInstance.getName() + ". Caused by: " + e.getMessage(), e);
            } else {
                log.error("Not found Network with extId: " + extId + " successfully on VimInstance " + vimInstance.getName() + ". Caused by: " + e.getMessage());
            }
            throw new VimException("Not found Network with extId: " + extId + " successfully on VimInstance " + vimInstance.getName() + ". Caused by: " + e.getMessage(), e);
        }
    }

    @Override
    @Async
    public Future<VNFCInstance> allocate(VimInstance vimInstance, VirtualDeploymentUnit vdu, VirtualNetworkFunctionRecord vnfr, VNFComponent vnfComponent, String userdata, Map<String, String> floatingIps) throws VimException, VimDriverException {
        ...
        log.info("Launching VM with parameters: " + hostname + " - " + image + " - " + flavorExtId + " - " + vimInstance.getKeyPair() + " - " + networks + " - " + vimInstance.getSecurityGroups());
    ...
    protected String chooseImage(Collection<String> vm_images, VimInstance vimInstance) throws VimException {
    ...
    protected VNFCInstance getVnfcInstanceFromServer(VimInstance vimInstance, VNFComponent vnfComponent, String hostname, Server server, VirtualDeploymentUnit vdu, Map<String, String> floatingIps, VirtualNetworkFunctionRecord vnfr) {
